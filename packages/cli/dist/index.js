 ...
 */
async function updateRelativeImportsOnly(code, originalDir, targetDir) {
...
program
    ...
    const fetchGithubFile = async (githubUrl) => {
        const rawUrl = githubUrl
            .replace("github.com", "raw.githubusercontent.com")
            .replace("/blob/", "/");
        const res = await fetch(rawUrl);
        if (!res.ok)
            throw new Error(`Could not fetch ${path.basename(githubUrl)}`);
        return res.text();
    };
    const s = p.spinner();
    s.start('Downloading and transforming source files...');
    try {
        const selectedData = AUTH_REGISTRY[stratergySelection];
        // Fetch all raw files in parallel
        const downloadedFiles = await Promise.all(selectedData.files.map(async (file) => {
        ...
        for (const file of downloadedFiles) {
            ...
            const transformedCode = await updateRelativeImportsOnly(file.rawCode, file.originalDir, path.dirname(file.userTarget) // Use the relative target dir for internal math
            );
