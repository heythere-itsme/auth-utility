...
export const GoogleAuthButton = ({ onClick, className }: GoogleAuthButtonProps) => {
  return (
    <button
      onClick={onClick}
      className={cn(
        // Desktop Layout (Rectangle)
        "flex items-center gap-3 transition-all duration-200 border shadow-sm font-medium text-sm",
        "h-11 w-full max-w-[320px] px-4 rounded-md justify-start",

        // Mobile Layout (Circle Override)
        "max-sm:w-12 max-sm:h-12 max-sm:rounded-full max-sm:justify-center max-sm:p-0",

        // Brand Styling (Light/Dark Mode)
